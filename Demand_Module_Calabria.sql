-- ==============================================================================
-- CONSUMPTION MODULE: CALABRIA REGION
...
-- Note: Customized for Calabria's regional building classifications.
-- ==============================================================================
...
CREATE SCHEMA IF NOT EXISTS calabria;
...
CREATE INDEX IF NOT EXISTS idx_calabria_buildings_geom ON calabria.calabria_buildings USING GIST(geom);
CREATE INDEX IF NOT EXISTS idx_prov_geom ON public.prov_2024 USING GIST(geom);
CREATE INDEX IF NOT EXISTS idx_c11_geom ON calabria.r18_censimento_2011 USING GIST(geom);
CREATE INDEX IF NOT EXISTS idx_c21_geom ON calabria.r18_censimento_2021 USING GIST(geom);
CREATE INDEX IF NOT EXISTS idx_cab_geom ON public.primarycabins_2025 USING GIST(geom);

ANALYZE calabria.calabria_buildings;

-- ------------------------------------------------------------------------------
-- 2. SPATIAL MAPPING (TOPOLOGICAL INTEGRATION)
-- ------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS calabria.calabria_building_spatial_mapping (
...
TRUNCATE TABLE calabria.calabria_building_spatial_mapping;
...
FROM calabria.calabria_buildings;
...
INSERT INTO calabria.calabria_building_spatial_mapping (building_fid, year, cod_prov, sez2011, sez2021, cod_ac)
...
LEFT JOIN public.prov_2024 p ON ST_Within(b.point_geom, p.geom)
LEFT JOIN calabria.r18_censimento_2011 c11 ON ST_Within(b.point_geom, c11.geom)
LEFT JOIN calabria.r18_censimento_2021 c21 ON ST_Within(b.point_geom, c21.geom)
LEFT JOIN public.primarycabins_2025 cab ON ST_Within(b.point_geom, cab.geom)
ORDER BY b.fid; 

-- 2.2 Nearest Neighbor Rescue Logic (for boundary edge cases)
UPDATE calabria.calabria_building_spatial_mapping m
SET 
...
FROM (
    ...
    FROM calabria.calabria_building_spatial_mapping m2
    JOIN temp_building_centroids b ON m2.building_fid = b.fid::text
    LEFT JOIN LATERAL (SELECT cod_prov::text FROM public.prov_2024 ORDER BY b.point_geom <-> geom LIMIT 1) p_near ON TRUE
    LEFT JOIN LATERAL (SELECT sez2011::text FROM calabria.r18_censimento_2011 ORDER BY b.point_geom <-> geom LIMIT 1) c11_near ON TRUE
    LEFT JOIN LATERAL (SELECT sez21_id::text FROM calabria.r18_censimento_2021 ORDER BY b.point_geom <-> geom LIMIT 1) c21_near ON TRUE
    LEFT JOIN LATERAL (SELECT cod_ac::text FROM public.primarycabins_2025 ORDER BY b.point_geom <-> geom LIMIT 1) cab_near ON TRUE
...
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS calabria.calabria_buildings_integrated;

-- Consolidate spatial, building, and census data into a single master table
CREATE TABLE calabria.calabria_buildings_integrated AS
SELECT 
    b.*,
    ...
    i21."A8" as a8
FROM calabria.calabria_buildings b
JOIN calabria.calabria_building_spatial_mapping m ON b.fid::text = TRIM(m.building_fid)
LEFT JOIN calabria.r18_indicatori_2011_sezioni i11 ON TRIM(m.sez2011) = TRIM(i11."SEZ2011"::text)
LEFT JOIN calabria.r18_indicatori_2021_sezioni i21 ON TRIM(m.sez2021) = TRIM(i21."SEZ21_ID"::text);

ALTER TABLE calabria.calabria_buildings_integrated 
...
UPDATE calabria.calabria_buildings_integrated
SET 
...
UPDATE calabria.calabria_buildings_integrated SET is_energy_relevant = FALSE;

-- Qualify buildings (Custom filters for Calabria dataset)
UPDATE calabria.calabria_buildings_integrated
SET is_energy_relevant = TRUE
WHERE area >= 50 
  AND final_height >= 3.0 
  AND (
        ( "classifica" NOT IN ('TM') ) AND 
        ( NOT ("edifc_uso" ILIKE '%Monumento%' OR "edifc_uso" ILIKE '%Fontana%' OR ("classifica" = 'TH' AND "edifc_uso" NOT ILIKE 'Edificio aereoportuale%')) ) AND
        ( "edifc_uso" NOT ILIKE 'Edificio diruto%' )
      )
...
WHERE is_energy_relevant = TRUE 
  AND ( "classifica" = 'RL' AND "edifc_uso" ILIKE 'Edificio res%' );

-- Mixed Residential/Commercial
UPDATE calabria.calabria_buildings_integrated
SET res_piano = CASE 
                  WHEN floors <= 1 THEN 0 
                  WHEN floors BETWEEN 2 AND 4 THEN floors - 1 
                  ELSE floors - 2 
                END,
    no_res_piano = CASE 
                  WHEN floors <= 1 THEN floors 
                  WHEN floors BETWEEN 2 AND 4 THEN 1 
                  ELSE 2 
                END
WHERE is_energy_relevant = TRUE 
  AND ( "classifica" = 'RL' AND "edifc_uso" ILIKE 'Edificio residenziale / sportivo%' );

-- Non-Residential
UPDATE calabria.calabria_buildings_integrated
SET no_res_piano = floors, res_piano = 0
WHERE is_energy_relevant = TRUE 
  AND "classifica" IN ('SP', 'PA', 'TC', 'TH')
  AND "classifica" != 'RL';
...
-- 4.4 Sectoral Assignment (Agriculture, Industry, Services)
UPDATE calabria.calabria_buildings_integrated
SET 
    agr_v = CASE WHEN is_energy_relevant = TRUE AND "classifica" = 'PA' THEN no_res_v ELSE 0 END,
    ind_v = CASE WHEN is_energy_relevant = TRUE AND "classifica" = 'SP' AND "edifc_uso" ILIKE ANY (ARRAY['%Edificio industriale%', '%capannone%', '%costruzione%']) THEN no_res_v ELSE 0 END,
    ser_v = CASE WHEN is_energy_relevant = TRUE AND (
            ("classifica" = 'TC' AND ("edifc_uso" ILIKE '%stazione%' OR "edifc_uso" ILIKE '%ferroviari%')) OR
            ("classifica" = 'TH' AND "edifc_uso" ILIKE 'Edificio aereoportuale%')
        ) THEN no_res_v ELSE 0 END
WHERE is_energy_relevant = TRUE AND no_res_v > 0;

-- Fallback safety net for generic non-residential buildings
UPDATE calabria.calabria_buildings_integrated
SET ser_v = no_res_v
...
WITH summary_2011 AS (
    ...
    FROM calabria.calabria_buildings_integrated WHERE is_energy_relevant = TRUE GROUP BY sez2011
)
UPDATE calabria.r18_indicatori_2011_sezioni AS i
...
WITH summary_2021 AS (
    ...
    FROM calabria.calabria_buildings_integrated WHERE is_energy_relevant = TRUE GROUP BY sez2021
)
UPDATE calabria.r18_indicatori_2021_sezioni AS i
...
UPDATE calabria.calabria_buildings_integrated AS b
SET total_agr_v_sez = i.total_agr_v_sez, total_ind_v_sez = i.total_ind_v_sez, total_ser_v_sez = i.total_ser_v_sez
FROM calabria.r18_indicatori_2011_sezioni AS i WHERE b.is_energy_relevant = TRUE AND b.sez2011::text = i."SEZ2011"::text;

UPDATE calabria.calabria_buildings_integrated AS b
SET total_res_v_sez = i.total_res_v_sez
FROM calabria.r18_indicatori_2021_sezioni AS i WHERE b.is_energy_relevant = TRUE AND b.sez2021::text = i."SEZ21_ID"::text;
...
UPDATE calabria.r18_indicatori_2011_sezioni
...
FROM calabria.r18_indicatori_2011_sezioni AS i11 WHERE b.sez2011::text = i11."SEZ2011"::text AND b.is_energy_relevant = TRUE;
...
FROM calabria.r18_indicatori_2021_sezioni AS i21 WHERE b.sez2021::text = i21."SEZ21_ID"::text AND b.is_energy_relevant = TRUE;

-- 5.5 Downscaling Economic Data (Employees based on ATECO)
ALTER TABLE calabria.r18_indicatori_2011_sezioni ADD COLUMN addetti_agri float8 DEFAULT 0, ADD COLUMN addetti_indu float8 DEFAULT 0, ADD COLUMN addetti_serv float8 DEFAULT 0;
ALTER TABLE calabria."18_attecon_sce_2011" ADD COLUMN settore_descrizione TEXT;

UPDATE calabria."18_attecon_sce_2011"
...
WITH agg_eco AS (
    ...
    FROM calabria."18_attecon_sce_2011" GROUP BY "PROCOM", "NSEZ"
)
UPDATE calabria.r18_indicatori_2011_sezioni AS i
...
FROM calabria.calabria_building_spatial_mapping AS m
JOIN calabria.prelievo_medio_dei_clienti_domestici AS a ON CAST(m.cod_prov AS int8) = CAST(a.code_prov AS int8)
WHERE b.fid::text = m.building_fid::text AND a.year = 2024 AND b.is_energy_relevant = TRUE;

-- Service Sector (Residual Method from ENEA - using correct cod_reg = 18 for Calabria)
WITH regional_metrics AS (
    SELECT "civile_GWh" * 1000000.0 AS total_civil_kwh FROM calabria.rapporto_annuale_sull_efficienza_energetica WHERE report_year = 2025 AND cod_reg = 18 LIMIT 1
),
assigned_totals AS (
    SELECT SUM(COALESCE(cons_res_kwh_yr, 0)) AS total_res_assigned, SUM(COALESCE(work_ser_11, 0)) AS total_emp_relevant
    FROM calabria.calabria_buildings_integrated WHERE is_energy_relevant = TRUE
),
factor_calc AS (
...
-- Industry & Agriculture (Proportional Downscaling - using correct cod_reg = 18 for Calabria)
WITH regional_source AS (
    SELECT "industria_GWh" AS total_ind_gwh, "agricoltura e pesca_GWh" AS total_agr_gwh 
    FROM calabria.rapporto_annuale_sull_efficienza_energetica WHERE report_year = 2025 AND cod_reg = 18 LIMIT 1
),
global_drivers AS (
...
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS calabria.calabria_building_hourly_consumption;

CREATE TABLE calabria.calabria_building_hourly_consumption (
...
BEGIN
    FOR m IN 1..12 LOOP
        ...
        FROM (
            ...
            FROM calabria.calabria_buildings_integrated b
            JOIN calabria.calabria_building_spatial_mapping sm ON b.fid::text = sm.building_fid::text
...
-- 8. FINAL INDEXING
-- ------------------------------------------------------------------------------
CREATE INDEX IF NOT EXISTS idx_cal_hourly_fid_final ON calabria.calabria_building_hourly_consumption(building_fid);
ANALYZE calabria.calabria_building_hourly_consumption;