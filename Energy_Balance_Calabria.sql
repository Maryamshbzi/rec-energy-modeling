...
-- primary substation (cabin) and provincial levels for the Calabria region.
-- ==============================================================================
...
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS calabria.cabin_hourly_demand;
CREATE UNLOGGED TABLE calabria.cabin_hourly_demand AS
SELECT
    sm.cod_ac,
    sm.cod_prov,
    h.month,
...
FROM calabria.calabria_building_hourly_consumption h
JOIN calabria.calabria_building_spatial_mapping sm 
...
CREATE INDEX idx_cabin_demand ON calabria.cabin_hourly_demand (cod_ac, month, fr_fs, hour);
ANALYZE calabria.cabin_hourly_demand;

-- ------------------------------------------------------------------------------
-- 3. UNNEST PRODUCTION ARRAYS TO LONG FORMAT (CABIN LEVEL)
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS calabria.cabin_hourly_supply;
CREATE UNLOGGED TABLE calabria.cabin_hourly_supply AS
SELECT
    m.cod_ac,
    m.cod_prov::int4,
    p.month,
...
FROM calabria.calabria_actual_plants_hourly_production p
JOIN calabria.calabria_actual_plants_spatial_mapping m 
...
CREATE INDEX idx_cabin_supply ON calabria.cabin_hourly_supply (cod_ac, month, fr_fs, hour);
ANALYZE calabria.cabin_hourly_supply;

-- ------------------------------------------------------------------------------
-- 4. COMBINE DEMAND & SUPPLY: CALCULATE SC, UD, OP PER HOUR (CABIN LEVEL)
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS calabria.cabin_hourly_balance;
CREATE UNLOGGED TABLE calabria.cabin_hourly_balance AS
SELECT
    d.cod_ac,
    d.cod_prov,
    d.month,
...
FROM calabria.cabin_hourly_demand d
LEFT JOIN calabria.cabin_hourly_supply s
...
CREATE INDEX idx_cabin_balance ON calabria.cabin_hourly_balance (cod_ac, month, fr_fs);
ANALYZE calabria.cabin_hourly_balance;

-- ------------------------------------------------------------------------------
-- 5. ANNUAL SCI & SSI — CABIN LEVEL
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS calabria.results_cabin;
CREATE TABLE calabria.results_cabin AS
SELECT
    cod_ac,
    cod_prov,
...
FROM calabria.cabin_hourly_balance
GROUP BY cod_ac, cod_prov
ORDER BY cod_ac;

CREATE INDEX idx_results_cabin_ac ON calabria.results_cabin (cod_ac);
CREATE INDEX idx_results_cabin_prov ON calabria.results_cabin (cod_prov);
ANALYZE calabria.results_cabin;

-- ------------------------------------------------------------------------------
-- 6. ANNUAL SCI & SSI — PROVINCE LEVEL
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS calabria.results_province;
CREATE TABLE calabria.results_province AS
SELECT
    cod_prov,
...
ANALYZE calabria.results_province;

-- ------------------------------------------------------------------------------
-- 7. MONTHLY SCI & SSI — CABIN LEVEL
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS calabria.results_cabin_monthly;
CREATE TABLE calabria.results_cabin_monthly AS
SELECT
    cod_ac,
    cod_prov,
    month,
...
ANALYZE calabria.results_cabin_monthly;
...
FROM calabria.results_cabin;

-- Province summary
SELECT * FROM calabria.results_province ORDER BY cod_prov;
...
DROP TABLE IF EXISTS calabria.advanced_results_cabin;
CREATE TABLE calabria.advanced_results_cabin AS
SELECT
    cod_ac,
    cod_prov,
    annual_demand_kwh,
...
CREATE INDEX idx_adv_results_cabin_ac ON calabria.advanced_results_cabin(cod_ac);
ANALYZE calabria.advanced_results_cabin;

-- EXTENDED RESULTS AT PROVINCE LEVEL (Values in GWh and Million Euros)
DROP TABLE IF EXISTS calabria.advanced_results_province;
CREATE TABLE calabria.advanced_results_province AS
SELECT
    cod_prov,
    annual_demand_gwh,
    annual_supply_gwh,
...
FROM calabria.results_province
ORDER BY cod_prov;

ANALYZE calabria.advanced_results_province;

-- Quick Overview of Advanced KPIs
SELECT 
    'CALABRIA TOTAL ADVANCED' AS region,
...
FROM calabria.advanced_results_province;
...
DROP TABLE IF EXISTS calabria.environmental_nexus_province;
CREATE TABLE calabria.environmental_nexus_province AS
SELECT
    cod_prov,
    annual_supply_gwh, 
...
ANALYZE calabria.environmental_nexus_province;

-- Quick Overview of Environmental Impact
SELECT 
    'CALABRIA TOTAL ENVIRONMENTAL IMPACT' AS region,
...
FROM calabria.environmental_nexus_province;
