...
-- primary substation (cabin) and provincial levels for the Puglia region.
-- ==============================================================================
...
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS puglia.cabin_hourly_demand;
CREATE UNLOGGED TABLE puglia.cabin_hourly_demand AS
SELECT
    sm.cod_ac,
    sm.cod_prov,
    h.month,
...
FROM puglia.puglia_building_hourly_consumption h
JOIN puglia.puglia_building_spatial_mapping sm 
...
CREATE INDEX idx_cabin_demand ON puglia.cabin_hourly_demand (cod_ac, month, fr_fs, hour);
ANALYZE puglia.cabin_hourly_demand;

-- ------------------------------------------------------------------------------
-- 3. UNNEST PRODUCTION ARRAYS TO LONG FORMAT (CABIN LEVEL)
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS puglia.cabin_hourly_supply;
CREATE UNLOGGED TABLE puglia.cabin_hourly_supply AS
SELECT
    m.cod_ac,
    m.cod_prov::int4,
    p.month,
...
FROM puglia.puglia_actual_plants_hourly_production p
JOIN puglia.puglia_actual_plants_spatial_mapping m 
...
CREATE INDEX idx_cabin_supply ON puglia.cabin_hourly_supply (cod_ac, month, fr_fs, hour);
ANALYZE puglia.cabin_hourly_supply;

-- ------------------------------------------------------------------------------
-- 4. COMBINE DEMAND & SUPPLY: CALCULATE SC, UD, OP PER HOUR (CABIN LEVEL)
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS puglia.cabin_hourly_balance;
CREATE UNLOGGED TABLE puglia.cabin_hourly_balance AS
SELECT
    d.cod_ac,
    d.cod_prov,
    d.month,
...
FROM puglia.cabin_hourly_demand d
LEFT JOIN puglia.cabin_hourly_supply s
...
CREATE INDEX idx_cabin_balance ON puglia.cabin_hourly_balance (cod_ac, month, fr_fs);
ANALYZE puglia.cabin_hourly_balance;

-- ------------------------------------------------------------------------------
-- 5. ANNUAL SCI & SSI — CABIN LEVEL
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS puglia.results_cabin;
CREATE TABLE puglia.results_cabin AS
SELECT
    cod_ac,
    cod_prov,
...
FROM puglia.cabin_hourly_balance
GROUP BY cod_ac, cod_prov
ORDER BY cod_ac;

CREATE INDEX idx_results_cabin_ac ON puglia.results_cabin (cod_ac);
CREATE INDEX idx_results_cabin_prov ON puglia.results_cabin (cod_prov);
ANALYZE puglia.results_cabin;

-- ------------------------------------------------------------------------------
-- 6. ANNUAL SCI & SSI — PROVINCE LEVEL
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS puglia.results_province;
CREATE TABLE puglia.results_province AS
SELECT
    cod_prov,
...
ANALYZE puglia.results_province;

-- ------------------------------------------------------------------------------
-- 7. MONTHLY SCI & SSI — CABIN LEVEL
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS puglia.results_cabin_monthly;
CREATE TABLE puglia.results_cabin_monthly AS
SELECT
    cod_ac,
    cod_prov,
    month,
...
ANALYZE puglia.results_cabin_monthly;
...
FROM puglia.results_cabin;

-- Province summary
SELECT * FROM puglia.results_province ORDER BY cod_prov;
...
DROP TABLE IF EXISTS puglia.advanced_results_cabin;
CREATE TABLE puglia.advanced_results_cabin AS
SELECT
    cod_ac,
    cod_prov,
    annual_demand_kwh,
...
CREATE INDEX idx_adv_results_cabin_ac ON puglia.advanced_results_cabin(cod_ac);
ANALYZE puglia.advanced_results_cabin;

-- EXTENDED RESULTS AT PROVINCE LEVEL (Values in GWh and Million Euros)
DROP TABLE IF EXISTS puglia.advanced_results_province;
CREATE TABLE puglia.advanced_results_province AS
SELECT
    cod_prov,
    annual_demand_gwh,
    annual_supply_gwh,
...
FROM puglia.results_province
ORDER BY cod_prov;

ANALYZE puglia.advanced_results_province;

-- Quick Overview of Advanced KPIs
SELECT 
    'PUGLIA TOTAL ADVANCED' AS region,
...
FROM puglia.advanced_results_province;
...
DROP TABLE IF EXISTS puglia.environmental_nexus_province;
CREATE TABLE puglia.environmental_nexus_province AS
SELECT
    cod_prov,
    annual_supply_gwh, 
...
ANALYZE puglia.environmental_nexus_province;

-- Quick Overview of Environmental Impact
SELECT 
    'PUGLIA TOTAL ENVIRONMENTAL IMPACT' AS region,
...
FROM puglia.environmental_nexus_province;
