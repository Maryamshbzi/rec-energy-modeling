"""
===========================================================================
TEMPORAL DISAGGREGATION PROFILE BUILDER - CALABRIA
===========================================================================
Description: 
...
# Provincial mapping codes for Calabria (Aligned with ISTAT COD_PROV standards)
PROV_ID_MAP = {
    'Cosenza': 78,
    'Catanzaro': 79,
    'Reggio di Calabria': 80,
    'Crotone': 101,
    'Vibo Valentia': 102
}
...
print(f"Executing Batch Conversion for {len(excel_files)} files in {REGION.capitalize()}...")
...
for file_path in res_files:
    try:
        ...
        # Robust province extraction from filenames like "Line - Ora 3 Catanzaro"
        prov_name = next((p for p in PROV_ID_MAP.keys() if p.lower() in filename.lower()), None)
        
        if not prov_name:
            continue # Skip files that don't contain a valid province name
