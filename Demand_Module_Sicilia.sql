-- ==============================================================================
-- CONSUMPTION MODULE: SICILIA REGION
...
CREATE SCHEMA IF NOT EXISTS sicilia;
...
CREATE INDEX IF NOT EXISTS idx_sicilia_buildings_geom ON sicilia.sicilia_buildings USING GIST(geom);
CREATE INDEX IF NOT EXISTS idx_prov_geom ON public.prov_2024 USING GIST(geom);
CREATE INDEX IF NOT EXISTS idx_c11_geom ON sicilia.r19_censimento_2011 USING GIST(geom);
CREATE INDEX IF NOT EXISTS idx_c21_geom ON sicilia.r19_censimento_2021 USING GIST(geom);
CREATE INDEX IF NOT EXISTS idx_cab_geom ON public.primarycabins_2025 USING GIST(geom);

ANALYZE sicilia.sicilia_buildings;

-- ------------------------------------------------------------------------------
-- 2. SPATIAL MAPPING (TOPOLOGICAL INTEGRATION)
-- ------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS sicilia.sicilia_building_spatial_mapping (
...
TRUNCATE TABLE sicilia.sicilia_building_spatial_mapping;
...
FROM sicilia.sicilia_buildings;
...
INSERT INTO sicilia.sicilia_building_spatial_mapping (building_fid, year, cod_prov, sez2011, sez2021, cod_ac)
...
LEFT JOIN sicilia.r19_censimento_2011 c11 ON ST_Within(b.point_geom, c11.geom)
LEFT JOIN sicilia.r19_censimento_2021 c21 ON ST_Within(b.point_geom, c21.geom)
...
UPDATE sicilia.sicilia_building_spatial_mapping m
SET 
...
FROM (
    ...
    FROM sicilia.sicilia_building_spatial_mapping m2
    ...
    LEFT JOIN LATERAL (SELECT sez2011::text FROM sicilia.r19_censimento_2011 ORDER BY b.point_geom <-> geom LIMIT 1) c11_near ON TRUE
    LEFT JOIN LATERAL (SELECT sez21_id::text FROM sicilia.r19_censimento_2021 ORDER BY b.point_geom <-> geom LIMIT 1) c21_near ON TRUE
...
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS sicilia.sicilia_buildings_integrated;

-- Consolidate spatial, building, and census data into a single master table
CREATE TABLE sicilia.sicilia_buildings_integrated AS
SELECT 
    b.*,
    ...
    i21."A8" as a8
FROM sicilia.sicilia_buildings b
JOIN sicilia.sicilia_building_spatial_mapping m ON b.fid::text = TRIM(m.building_fid)
LEFT JOIN sicilia.r19_indicatori_2011_sezioni i11 ON TRIM(m.sez2011) = TRIM(i11."SEZ2011"::text)
LEFT JOIN sicilia.r19_indicatori_2021_sezioni i21 ON TRIM(m.sez2021) = TRIM(i21."SEZ21_ID"::text);

ALTER TABLE sicilia.sicilia_buildings_integrated 
...
UPDATE sicilia.sicilia_buildings_integrated
SET 
...
SET final_height = CASE 
    WHEN h_regional >= 2.5 AND h_regional <= 150 THEN h_regional
...
UPDATE sicilia.sicilia_buildings_integrated SET is_energy_relevant = FALSE;
UPDATE sicilia.sicilia_buildings_integrated
SET is_energy_relevant = TRUE
WHERE area >= 50 
  AND final_height >= 3.0 
  AND classifica NOT IN ('Others', 'TC', 'TI')
  AND floors IS NOT NULL;

-- 4.2 Floor split logic
UPDATE sicilia.sicilia_buildings_integrated
SET res_piano = floors, no_res_piano = 0
WHERE is_energy_relevant = TRUE AND (classifica = 'R' OR edifc_uso ILIKE '%Unità%Volumetrica%civile%');

UPDATE sicilia.sicilia_buildings_integrated
SET no_res_piano = floors, res_piano = 0
WHERE is_energy_relevant = TRUE AND classifica IN ('SP', 'PA');
...
UPDATE sicilia.sicilia_buildings_integrated
SET 
    agr_v = CASE WHEN is_energy_relevant = TRUE AND (classifica = 'PA' OR edifc_uso ILIKE ANY (ARRAY['%Serra stabile%', '%Stalla%', '%fienile%'])) THEN no_res_v ELSE 0 END,
    ind_v = CASE WHEN is_energy_relevant = TRUE AND classifica = 'SP' AND edifc_uso ILIKE ANY (ARRAY['%Torre%', '%ciminiera%', '%silos%', '%Centrale elettrica%', '%cabina elettrica%']) THEN no_res_v ELSE 0 END,
    ser_v = CASE WHEN is_energy_relevant = TRUE AND classifica = 'SP' AND edifc_uso ILIKE ANY (ARRAY['%Unità Volumetrica industriale%', '%Unità volumetrica commerciale%', '%Capannone%']) THEN no_res_v ELSE 0 END
WHERE is_energy_relevant = TRUE AND no_res_v > 0;

-- Fallback for unclassified service structures
UPDATE sicilia.sicilia_buildings_integrated
SET ser_v = no_res_v
WHERE is_energy_relevant = TRUE AND classifica = 'SP' AND ind_v = 0 AND ser_v = 0 AND no_res_v > 0;
...
WITH summary_2011 AS (
    ...
    FROM sicilia.sicilia_buildings_integrated WHERE is_energy_relevant = TRUE GROUP BY sez2011
)
UPDATE sicilia.r19_indicatori_2011_sezioni AS i
...
WITH summary_2021 AS (
    ...
    FROM sicilia.sicilia_buildings_integrated WHERE is_energy_relevant = TRUE GROUP BY sez2021
)
UPDATE sicilia.r19_indicatori_2021_sezioni AS i
...
UPDATE sicilia.sicilia_buildings_integrated AS b
SET total_agr_v_sez = i.total_agr_v_sez, total_ind_v_sez = i.total_ind_v_sez, total_ser_v_sez = i.total_ser_v_sez
FROM sicilia.r19_indicatori_2011_sezioni AS i WHERE b.is_energy_relevant = TRUE AND b.sez2011::text = i."SEZ2011"::text;

UPDATE sicilia.sicilia_buildings_integrated AS b
SET total_res_v_sez = i.total_res_v_sez
FROM sicilia.r19_indicatori_2021_sezioni AS i WHERE b.is_energy_relevant = TRUE AND b.sez2021::text = i."SEZ21_ID"::text;

-- 5.3 Safety Net for Data Imputation (Zero-denominator prevention)
-- If a building still has a zero denominator, use its own volume to force a 100% allocation.
UPDATE sicilia.sicilia_buildings_integrated
SET 
...
UPDATE sicilia.r19_indicatori_2011_sezioni
...
FROM sicilia.r19_indicatori_2011_sezioni AS i11 WHERE b.sez2011::text = i11."SEZ2011"::text AND b.is_energy_relevant = TRUE;
...
FROM sicilia.r19_indicatori_2021_sezioni AS i21 WHERE b.sez2021::text = i21."SEZ21_ID"::text AND b.is_energy_relevant = TRUE;

-- 5.5 Downscaling Economic Data (Employees based on ATECO)
ALTER TABLE sicilia.r19_indicatori_2011_sezioni ADD COLUMN addetti_agri float8 DEFAULT 0, ADD COLUMN addetti_indu float8 DEFAULT 0, ADD COLUMN addetti_serv float8 DEFAULT 0;
ALTER TABLE sicilia."19_attecon_sce_2011" ADD COLUMN settore_descrizione TEXT;

UPDATE sicilia."19_attecon_sce_2011"
...
WITH agg_eco AS (
    ...
    FROM sicilia."19_attecon_sce_2011" GROUP BY "PROCOM", "NSEZ"
)
UPDATE sicilia.r19_indicatori_2011_sezioni AS i
...
FROM sicilia.sicilia_building_spatial_mapping AS m
JOIN sicilia.prelievo_medio_dei_clienti_domestici AS a ON CAST(m.cod_prov AS int8) = CAST(a.code_prov AS int8)
WHERE b.fid::text = m.building_fid::text AND a.year = 2024 AND b.is_energy_relevant = TRUE;

-- Service Sector (Residual Method from ENEA)
WITH regional_metrics AS (
    SELECT "civile_GWh" * 1000000.0 AS total_civil_kwh FROM sicilia.rapporto_annuale_sull_efficienza_energetica WHERE report_year = 2025 AND cod_reg = 19 LIMIT 1
),
assigned_totals AS (
    SELECT SUM(COALESCE(cons_res_kwh_yr, 0)) AS total_res_assigned, SUM(COALESCE(work_ser_11, 0)) AS total_emp_relevant
    FROM sicilia.sicilia_buildings_integrated WHERE is_energy_relevant = TRUE
),
factor_calc AS (
...
WITH regional_source AS (
    ...
    FROM sicilia.rapporto_annuale_sull_efficienza_energetica WHERE report_year = 2025 AND cod_reg = 19 LIMIT 1
),
global_drivers AS (
...
-- ------------------------------------------------------------------------------
DROP TABLE IF EXISTS sicilia.sicilia_building_hourly_consumption;

CREATE TABLE sicilia.sicilia_building_hourly_consumption (
...
BEGIN
    FOR m IN 1..12 LOOP
        ...
        FROM (
            ...
            FROM sicilia.sicilia_buildings_integrated b
            JOIN sicilia.sicilia_building_spatial_mapping sm ON b.fid::text = sm.building_fid::text
...
-- 8. FINAL INDEXING
-- ------------------------------------------------------------------------------
CREATE INDEX idx_sic_hourly_fid_final ON sicilia.sicilia_building_hourly_consumption(building_fid);
ANALYZE sicilia.sicilia_building_hourly_consumption;